...
async function carregarCategorias() {
    try {
        ...
        exibirMensagem(document.getElementById("mensagem-produto"), 'Erro ao carregar categorias', 'erro');
    }
}
    ...
    try {
        ...
        produtos.forEach(produto => {
            ...
            newRow.innerHTML = `
                ...
                <td class="${produto.quantidade <= 5 ? "baixo-estoque" : ""}">${produto.quantidade}</td>
                <td class="acoes">
        ...
        exibirMensagem(document.getElementById("mensagem-produto"), 'Erro ao atualizar tabela', 'erro');
    }
}
...
async function atualizarTabelaMovimentacoes() {
    try {
        ...
        exibirMensagem(document.getElementById("mensagem-movimentacao"), 'Erro ao atualizar movimentações', 'erro');
    }
}
...
document.getElementById("produto-form").addEventListener("submit", async function (event) {
    ...
    try {
        ...
        this.reset();
        exibirMensagem(document.getElementById("mensagem-produto"), "Produto adicionado com sucesso!", "sucesso");
    } catch (error) {
        console.error('Erro:', error);
        exibirMensagem(document.getElementById("mensagem-produto"), "Erro ao adicionar produto!", "erro");
    }
});
...
document.getElementById("movimentacao-form").addEventListener("submit", async function (event) {
    ...
    try {
        ...
        await atualizarTabelaProdutos();
        this.reset();
        exibirMensagem(document.getElementById("mensagem-movimentacao"), "Movimentação registrada com sucesso!", "sucesso");
    } catch (error) {
        console.error('Erro:', error);
        exibirMensagem(document.getElementById("mensagem-movimentacao"), error.message, "erro");
    }
});
...
async function atualizarTotalEstoque() {
    try {
        ...
        const produtos = await response.json();
        const totalQuantidade = produtos.reduce((total, produto) => total + produto.quantidade, 0);
        ...
        showModal(`
            ...
            <form id="editar-produto-form" onsubmit="salvarEdicaoProduto(event, '${id}')">
                <div class="form-group">
                    <label>Tipo:</label>
                    <input type="text" id="edit-tipo" value="${produto.tipo}" required>
                    ...
                    <label>Modelo:</label>
                    <input type="text" id="edit-modelo" value="${produto.modelo}" required>
                    
                    <label>Número de Série:</label>
                    <input type="text" id="edit-serie" value="${produto.numero_serie || ''}">
                    
                    <label>Patrimônio:</label>
                    <input type="text" id="edit-patrimonio" value="${produto.patrimonio || ''}">
                    
                    <label>Quantidade:</label>
                    <input type="number" id="edit-quantidade" value="${produto.quantidade}" required min="0">
                </div>
...
async function excluirProduto(id) {
    ...
    try {
        ...
        const responseMovimentacoes = await fetch(`/api/produtos/${id}/movimentacoes`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
...
async function carregarOpcoesCategoria(categoriaAtual) {
    try {
        ...
        return categorias.map(cat => `
            <option value="${cat.id}" ${cat.id === categoriaAtual ? 'selected' : ''}>
                ${cat.nome}
            </option>
        `).join('');
    } catch (error) {
...
function exportarDados(formato) {
    if (formato === "csv") {
        ...
        csvContent += "ID;Categoria;Tipo;Modelo;Número de Série;Quantidade em Estoque\n";
        ...
        csvContent += "Data;Usuário;Categoria;Tipo;Modelo;Número de Série;Quantidade;Tipo Mov.;Destino;Chamado\n";

        movimentacoes.forEach((mov) => {
            csvContent += `${mov.data};${mov.usuario};${mov.categoria};${mov.tipoProd};${mov.modelo};${mov.serie};${mov.quantidade};${mov.tipo};${mov.destino};${mov.chamado || 'N/A'}\n`;
        });
        ...
        let link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", `inventario_estoque_${new Date().toLocaleDateString().replace(/\//g, '-')}.csv`);
        document.body.appendChild(link);
        link.click();
        ...
        link.download = `inventario_estoque_${new Date().toLocaleDateString().replace(/\//g, '-')}.json`;
        link.click();
    }
}
...
function calcularTotalItensEstoque() {
    return Object.values(produtos).reduce((total, produto) => total + produto.quantidade, 0);
}

function calcularMovimentacoesHoje(tipo) {
    const hoje = new Date().toLocaleDateString();
    return movimentacoes.filter(mov => 
        mov.data.includes(hoje) && mov.tipo === tipo
    ).length;
}
...
function gerarRelatorio(tipo) {
    ...
    let conteudoRelatorio = `
        <h2>${titulo}</h2>
        ${tipo === 'criticos' ? gerarTabelaCriticos(relatorio) : gerarTabelaMovimentacoes(relatorio)}
...
function gerarResumoRelatorio(tipo, dados) {
    ...
    if (tipo === 'diario' || tipo === 'mensal') {
        const entradas = dados.filter(m => m.tipo === 'entrada')
            .reduce((total, m) => total + m.quantidade, 0);
        const saidas = dados.filter(m => m.tipo === 'saida')
            .reduce((total, m) => total + m.quantidade, 0);
        ...
        resumo = `
            <div class="resumo">
                ...
                <p>Usuários Diferentes: ${new Set(dados.map(m => m.usuario)).size}</p>
            </div>
        `;
        ...
        resumo = `
            <div class="resumo">
                ...
                <p>Categorias Afetadas: ${new Set(dados.map(i => i.categoria)).size}</p>
            </div>
        `;
    }
...
function verificarNotificacoes() {
    ...
    const itensCriticos = Object.entries(produtos)
        .filter(([_, produto]) => produto.quantidade <= thresholdValue);
...
async function buscarProdutos() {
    ...
    const modelo = document.getElementById('modelo-busca').value.toLowerCase().trim();
    const patrimonio = document.getElementById('patrimonio-busca').value.toLowerCase().trim();
    const serie = document.getElementById('serie-busca').value.toLowerCase().trim();
    ...
    try {
        ...
        produtos.forEach(produto => {
            ...
            if ((!tipo || produtoTipo.includes(tipo)) && 
        ...
        exibirMensagem(document.getElementById("mensagem-produto"), 'Erro ao buscar produtos', 'erro');
    }
}
...
function atualizarTabelaResultados(resultados) {
    ...
    for (let chave in resultados) {
        ...
        row.innerHTML = `
            ...
            <td class="${item.quantidade <= 5 ? 'baixo-estoque' : ''}">${item.quantidade}</td>  <!-- Quantidade -->
...
function mostrarResumoResultados(resultados) {
    const totalItens = Object.values(resultados).reduce((sum, item) => sum + item.quantidade, 0);
...
document.getElementById('modelo-busca').addEventListener('input', buscarProdutos);
document.getElementById('patrimonio-busca').addEventListener('input', buscarProdutos);
document.getElementById('serie-busca').addEventListener('input', buscarProdutos);
...
document.getElementById('produto-form').addEventListener('submit', function(event) {
    ...
    if (categoria.toLowerCase() !== 'perifericos' && !patrimonio && !serie) {
        exibirMensagem(document.getElementById("mensagem-produto"), 'Você deve preencher pelo menos um dos campos: Patrimônio ou Número de Série', 'erro');
        return;
    }
...
async function gerarRelatorio(event) {
    ...
    try {
        ...
        console.error('Erro:', error);
        exibirMensagem(document.getElementById("mensagem-relatorio"), 'Erro ao gerar relatório', 'erro');
    }
}
...
function atualizarResumoGeral(dados) {
    document.getElementById('total-produtos-relatorio').textContent = dados.totalProdutos.toLocaleString();
    document.getElementById('total-categorias').textContent = dados.totalCategorias.toLocaleString();
    document.getElementById('movimentacoes-dia').textContent = dados.movimentacoesDia.toLocaleString();
    document.getElementById('valor-total-estoque').textContent = 
        `R$ ${dados.valorTotalEstoque.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
}
...
function atualizarAlertas(alertas) {
    ...
    container.innerHTML = alertas.map(alerta => `
        <div class="alerta-item ${alerta.tipo}">
            <i class="fas ${alerta.tipo === 'critico' ? 'fa-exclamation-circle' : 'fa-info-circle'}"></i>
...
function atualizarProdutosCriticos(produtos) {
    ...
    if (produtos.length === 0) {
        container.innerHTML = '<div class="sem-dados">Nenhum produto em estado crítico</div>';
        return;
    }
            ...
            <tbody>
                ${produtos.map(p => `
                    <tr>
                        ...
                        <td class="quantidade ${p.quantidade <= 3 ? 'critico' : 'alerta'}">${p.quantidade}</td>
                        <td>
                            <span class="status-badge ${p.quantidade <= 3 ? 'critico' : 'alerta'}">
...
async function exportarRelatorioCompleto() {
    try {
        ...
        XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(dados.produtosCriticos), "Estoque Crítico");
        XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(dados.ultimasMovimentacoes), "Movimentações");
        XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(dados.distribuicaoCategorias), "Categorias");
...
async function gerarRelatorioMovimentacoes() {
    ...
    try {
        const response = await fetch(`/api/relatorios/movimentacoes?dataInicio=${dataInicio}&dataFim=${dataFim}`);
...
async function atualizarRelatorio() {
    try {
        ...
        if (movimentacoes.length === 0) {
            container.innerHTML = '<div class="sem-dados">Nenhuma movimentação encontrada.</div>';
            return;
        }
...
function exportarRelatorio() {
    ...
    const ws = XLSX.utils.table_to_sheet(tabela);
    XLSX.utils.book_append_sheet(wb, ws, "Movimentações");
    XLSX.writeFile(wb, `relatorio_movimentacoes_${new Date().toLocaleDateString()}.xlsx`);
}
